...
class DaLUKE(nn.Module):
    ...
    def forward(self, example: Example) -> (torch.Tensor, torch.Tensor):
        """
        """
        word_hidden    = self.word_embeddings(example.words)
        entity_hidden  = self.entity_embeddings(example.entities)
        attention_mask = self.attention_mask(example.words.attention_mask, example.entities.attention_mask)

        for encode in self.encoder:
            word_hidden, entity_hidden = encode(word_hidden, entity_hidden)
...
class EntitySelfAttention(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, drop_prob: float):
        ...
        self.Q_w, self.Q_e, self.Q_w2e, self.Q_e2w, self.K, self.V =
            nn.Linear(hidden_size, self.num_heads*self.head_size) for _ in range(6)
    ...
    def forward(self, word_hidden: torch.Tensor, entity_hidden: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        context = (attention_probs @ value).permute(0, 2, 1, 3).contiguous()
        return context.view(
